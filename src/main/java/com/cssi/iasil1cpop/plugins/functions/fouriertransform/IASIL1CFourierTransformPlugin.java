...
public class IASIL1CFourierTransformPlugin
	...
	/**
	 * Grabs and validates inputs necessary to call the IASI L1C Fourier
	 * Transform algorithm
	 * 
	 * @param input
	 *            the tuple of data transfered by the DPI to the plugin
	 * @return a CheckedInput containing the IASI L1C Fourier Transform inputs
	 *         and an error report
	 */
	@Override
	 ...
	 * 
	 * @param the
	 *            algorithm output object
	 * @return the objects to transform into tuple and the associated error
	 *         report
	 */
	@Override
	public List<CheckedValue<Object>> checkOutputs(IASL1CFourierOutput ouput, ErrorReport errorReport) {
		...
		if (ouput.getInterferogram() == null || ouput.getInterferogram().length() == 0) {
			errorReport.addError(new DataIntegrityError(ErrorSeverity.WARNING,
					"Null Interferogram produced by Fourier transform", "Interferogram", ""));
		}
	...
	private void checkDataIntegriy(ComplexArray spk, Integer first, Integer last, Integer nfft,
		...
		if (nfft != spk.length()) {
			errorReport.addError(new DataIntegrityError(ErrorSeverity.WARNING,
					"SGA1_IAS_1C_AUX_CNF_ NFT field mismatches OGP_SPKCO size", "OGP_SPKCO", ""));
		}
		if (first < 0 || first >= nfft) {
			errorReport.addError(new AuxiliaryDataIntegrityError(ErrorSeverity.CRITICAL,
					String.format("Parameter first should be between 0 and %d", spk.length()), "SGA1_IAS_1C_AUX_CNF_",
					"FIRST_SCIENCE"));
		}
		if (last < 0 || last >= nfft) {
			errorReport.addError(new AuxiliaryDataIntegrityError(ErrorSeverity.CRITICAL,
					String.format("Parameter last should be between 0 and %d", spk.length()), "SGA1_IAS_1C_AUX_CNF_",
					"FIRST_SCIENCE"));
		}
		if (first >= last) {
			errorReport.addError(new AuxiliaryDataIntegrityError(ErrorSeverity.CRITICAL,
					"Parameter first should be inferior to parameter last", "SGA1_IAS_1C_AUX_CNF_", "FIRST_SCIENCE"));
		}
	}
	...
	private void checkDataCompletness(ITupleApi input, List<ITupleApi> aux_data, ErrorReport errorReport) {
		if (!input.contains("OGP_SPKCO")) {
			errorReport.addError(new DataMissingError(ErrorSeverity.CRITICAL, "Missing OGP_SPKCO tuple", "OGP_SPKCO"));
		}

		if (aux_data.isEmpty() || aux_data.size() > 1) {
			errorReport.addError(new AuxiliaryDataMissingError(ErrorSeverity.CRITICAL,
					"Auxiliary SGA1_IAS_1C_AUX_CNF_ not found", "SGA1_IAS_1C_AUX_CNF_"));
		}
		ITupleApi aux_cnf = aux_data.get(0);
		if (aux_cnf.contains("FIRST_SCIENCE")) {
			errorReport.addError(new AuxiliaryDataIntegrityError(ErrorSeverity.CRITICAL, "Field FIRST_SCIENCE Missing",
					"SGA1_IAS_1C_AUX_CNF_", "FIRST_SCIENCE"));
		}
		if (aux_cnf.contains("LAST_SCIENCE")) {
			errorReport.addError(new AuxiliaryDataIntegrityError(ErrorSeverity.CRITICAL, "Field LAST_SCIENCE Missing",
					"SGA1_IAS_1C_AUX_CNF_", "LAST_SCIENCE"));
		}
		if (aux_cnf.contains("DOPD")) {
			errorReport.addError(new AuxiliaryDataIntegrityError(ErrorSeverity.WARNING, "Field DOPD Missing",
					"SGA1_IAS_1C_AUX_CNF_", "DOPD"));
		}
		if (aux_cnf.contains("NFFT")) {
			errorReport.addError(new AuxiliaryDataIntegrityError(ErrorSeverity.ERROR, "Field NFFT Missing",
					"SGA1_IAS_1C_AUX_CNF_", "NFFT"));
		}
	}
